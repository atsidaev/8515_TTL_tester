...
void test_74166(void)
    {   
        ...
        Port_H=(P_nRST | P_VCC);
        delay_ms(1);
        if ((Pin_L==0) && (Pin_H==(P_nRST | P_VCC))) res=1;
            else res=0;
        ...
        Port_L=0;
        delay_ms(1);
        if ((Pin_L==0) && (Pin_H==(P_nRST | P_SH_LD | P_VCC)) && (res==1)) res=1;
            else res=0; 
        Port_L=(P_CLK2);
        Port_L=0;
        delay_ms(1);
        if ((Pin_H==(P_nRST | P_Q7 | P_SH_LD | P_VCC)) && (res==1)) res=1;
            else res=0;
        Port_L=(P_CLK2);
        Port_L=0;
        if ((Pin_H==(P_nRST | P_SH_LD | P_VCC)) && (res==1)) res=1;
            else res=0;        
        if (res==1) sym[0]=10;
            else sym[0]=11;
        Port_L=0;
