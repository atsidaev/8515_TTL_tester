...
void test_74244(void)
    {
        ...
        PORTE=0;
        if ((Pin_L==(P_1A1 | P_2Y4 | P_1A3 | P_2Y2)) && (Pin_H==(P_2A2 | P_1Y3 | P_2A4 | P_1Y1 | P_VCC)) && (PINE==0)) res=1;
            else res=0;
        Port_L=(P_1A2 | P_1A4);
        Port_H=(P_2A3 | P_VCC);
        PORTE=2;
        if ((Pin_L==(P_1A2 | P_2Y3 | P_1A4)) && (Pin_H==(P_2A3 | P_1Y2 | P_VCC)) && (PINE==7) && (res==1)) res=1;
            else res=0;    
        if (res==1) sym[0]=10;
            else sym[0]=11;
        Port_L=0;
